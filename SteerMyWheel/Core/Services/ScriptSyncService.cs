...
namespace SteerMyWheel.Core.Services
{
    public class ScriptSyncService 
    {
        ...
        private IEnumerable<ScriptRepository> generateMigrationWorkers(IEnumerable<ScriptRepository> repositories)
        {
            ...
            var verify = Task.Run(() =>
            {
                var list = Directory.GetDirectories(_config.LocalReposDirectory);
                using (var client = _client)
                {
                    foreach (var dir in list)
                    {
                        var name = dir.Replace(_config.LocalReposDirectory, "");
        ...
        private IEnumerable<ScriptExecution> getAllScripts(RemoteHost host)
        {
            ...
            var scripts = _repositories.ScriptExecutionRepository.GetAll(host);
